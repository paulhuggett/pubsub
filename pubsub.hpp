...
namespace pubsub {
  ...
  public:
    ...
    /// \param rel_time A duration representing the maximum time to spend waiting. Note that
    /// rel_time must be small enough not to overflow when added to
    /// std::chrono::steady_clock::now().
    template <typename Rep, typename Period>
    std::optional<Message> wait_for (std::chrono::duration<Rep, Period> const & rel_time);

    /// \param timeout_time	-	An object of type std::chrono::time_point representing the
  ...
  private:
    template <typename WaitFunction,
              typename = std::enable_if_t<std::is_invocable_r_v<
                bool, WaitFunction, std::condition_variable &, std::unique_lock<std::mutex> &>>>
  ...
  template <typename Rep, typename Period>
  std::optional<Message>
  subscriber<Message>::wait_for (std::chrono::duration<Rep, Period> const & rel_time) {
  ...
  template <typename Clock, typename Duration>
  std::optional<Message>
  subscriber<Message>::wait_until (std::chrono::time_point<Clock, Duration> const & timeout_time) {
    return owner_->wait (
      *this, [&timeout_time] (std::condition_variable & cv, std::unique_lock<std::mutex> & lock) {
        return cv.wait_until (lock, timeout_time) == std::cv_status::no_timeout;
      });
  }
